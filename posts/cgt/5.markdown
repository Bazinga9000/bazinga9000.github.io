...
*Base Case*: $\ast0 + \ast0 = 0 + 0 = 0 = \ast0$

*Inductive Case*:

Suppose that, for all $b$ and $a$ where $b < a < k$, that $\ast(2^b) + \ast(2^a) = \ast(2^b + 2^a)$.
...
1) If the first player moves $\ast(2^a) \to \ast x$ for some $x$, the second player can move $\ast(2^a + 2^k) \to \ast x + \ast (2^k)$ (this sum is a nimber $\star y$ where $y < k$ by induction, so this is possible)
